...
- Aishwarya Rao : Business Analyst/ Backend Developer
- Nancy Samuel : Data Analyst/ Scientist
- Christina Ming: Business Analyst
...
##### Steps involved:
1. Connect Databricks through Microsoft Azure, then lauch AutoML experiment and import the dataset.
2. Set the F1 score ass the performance metrics.
3. Databricks will automatically configure, train and evaluate. 
4. Choose the best model (in our case is the XGBoost based on the highest validation F1 score)(https://github.com/McGill-MMA-EnterpriseAnalytics/Auto_Insurance_Claims/blob/main/AutoML/Databricks/Best_Model_XGBoost_AutoML.ipynb)
5. Register the best model into model registry.
6. Deploy the model using MLflow model deployment (https://github.com/McGill-MMA-EnterpriseAnalytics/Auto_Insurance_Claims/blob/main/AutoML/Databricks/Azure%20Databricks%20AutoML.ipynb).

#### Advantages of using Databricks AutoML:
1. Automated data preprocessing, feature engineering, model training and hyperparameter tuning.
2. Automatically generated editable notebook for further customization.
3. Using MLflow modeling serving, it allows us to host our registered model from Model Registry as REST endpoints that are updated automatically based on the updated model versions.

#### Databricks AutoML Performances:
AutoML checked 5 models in total, and chose XGBoost as our final model because it has the highest validation F1 score of 91.1%. Compared with the models that we built last term, the F1 score of XGBoost model generated by AutoML increased by 68.1%, recall increased by 0.9%, precision increased by 75.3% and test accuracy increase by 31.5%.  
<img width="590" alt="Screen Shot 2022-04-21 at 2 09 09 PM" src="https://user-images.githubusercontent.com/90296247/164525054-d8d7278f-ab71-4c2e-a649-7b34f53d9e53.png">
